...
| `SpeciesGrid` | Three columns normally; two when width or font scale requires it |
...
Species Detail resolves reference media in this order: Wikimedia Commons Featured/Quality image associated with the linked Wikipedia article, licensed iNaturalist taxon default, compatible research-grade iNaturalist observation photo, then the closest licensed silhouette in species, genus, family, order and broad-group order. Collection cards still favour the user's own observation photo. Do not silently fall back to an unlicensed URL, an AI-generated animal or an unrelated generic animal image. Persist and expose the selected asset's source, attribution, licence, quality assessment and taxonomic match rank.
...
- **Complete:** Collection migrated from programmatic Views to Compose and verified on device with the real 57-entry / 89-observation cache. It uses real user observation photographs, accessible microscope icons for research-grade observations, near-square 3-column cards with a 2-column large-font fallback, search and filters, and explicit unlinked/empty/error/filter-empty states. The oversized summary panel was removed in favour of compact truthful stats; no completion percentage appears before the catalogue denominator is curated.
...
- **Complete:** Core sync no longer depends on a local or hosted Wildlife backend. Android directly reads the public iNaturalist and PhyloPic APIs through an on-device repository, conservatively paces requests, stores observation/catalogue/taxon projections in SQLite and records confirmation XP in an idempotent local ledger. App startup refreshes the linked user's public observations; cached screens remain usable offline. A future social service is optional and outside this core data path.
- **Complete:** Species Detail now separates curated reference media from personal sightings. It prefers strictly licensed Wikimedia Commons Featured/Quality article images and links an on-image attribution overlay to the Commons file page; iNaturalist defaults, compatible observation photos and silhouettes remain ordered fallbacks. Media pipeline v6 repairs literal spaces in the Wikipedia URLs returned by iNaturalist, sends the identifying request header Wikimedia requires when displaying the selected image, and refreshes older detail caches once.
...
- It works with TalkBack labels and large font scaling; the collection grid can fall back to two columns.
