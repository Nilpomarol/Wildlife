...
- **Complete:** Placeholder progression v0.1 centralizes enabled XP values and level thresholds, migrates existing ledger events without rewriting points, projects lifetime XP into a level, and adds a Profile surface with progress, recent reward sources and selectable earned titles. Rarity, badges, streaks and Research Grade XP remain disabled as documented.
- **Next product slice:** observation lifecycle synchronization and durable pending-handoff recovery. Its UI must expose last checked, stale/syncing/retry state and delayed quality transitions without presenting Wildlife confirmation as iNaturalist Research Grade. The manual Gate 1 field matrix remains open independently.
