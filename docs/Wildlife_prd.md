...
- The current region comes only from a one-shot device location assigned through the bundled boundaries. It is not manually editable. Explore may remember a different browsed guide, but that never changes regional progress or current-region cache priority. Without a usable fix, show unavailable/last-known state rather than silently choosing a region.
