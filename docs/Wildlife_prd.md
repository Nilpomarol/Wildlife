...
- **Every confirmed species-level observation counts as a personal discovery, whether or not the species belongs to a regional catalogue.** An off-catalogue species enters Collection normally, uses the user's photograph when available, earns the same global first-species reward and is labelled as an **Extra discovery** for its assigned region. Wildlife must never make a valid sighting feel worthless because editorial catalogue membership is narrower than nature.
...
- A regional guide includes **Extra discoveries** in the catalogue grid and reports their count additively alongside the frozen catalogue size (for example, `800 + 3 extra`). Extras may be isolated from the filter sheet, but never increase the catalogue denominator, completion percentage, rarity progress, Essentials or Icons.
...
- Species detail: scientific data, conservation status, Wikipedia summary, global distribution map versus the user's own points. Every species-level observation can open a detail page even when its taxon belongs to no installed catalogue; the page uses stored observation data immediately and may build its cached read-only taxon record on first open.
...
The **Extra discovery** vertical slice is implemented: confirmed off-catalogue species project into Collection with typed observation-region/catalogue-version context, receive the existing idempotent global first-species XP, and join the regional guide grid with an additive count and source filter without changing catalogue completion. Continue catalogue/media curation, representative-device release checks and shared UI consolidation.
