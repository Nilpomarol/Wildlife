...
EVENT_KEYS = (
    ...
    "regional_icon_discovery", "regional_essentials", "regional_icons", "identification_given",
    "anomaly_confirmed",
)
...
def load_workbook_content(workbook_path: Path) -> tuple[dict, dict, dict]:
    ...
    for index, row in enumerate(rows(workbook["Catalogue"]), start=2):
        ...
        if rarity not in RARITIES:
            fail(f"Catalogue row {index} needs a supported encounter rarity")
