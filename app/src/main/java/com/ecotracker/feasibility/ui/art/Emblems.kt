...
/**
 * The app's hand-drawn scout-patch emblem set.
 *
 * These are paths rather than Material icons so the badge language belongs to Wildlife
 * instead of reading as generic UI furniture, and so every mark tints with the theme
...
fun DrawScope.drawPaw(color: Color, center: Offset, radius: Float) {
    ...
    // Outer toes sit lower and smaller, which is what makes the arc read as a paw.
...
fun DrawScope.drawPine(color: Color, baseCenter: Offset, width: Float, height: Float) {
    ...
    listOf(0.00f to 1.00f, 0.30f to 0.78f, 0.58f to 0.54f).forEach { (lift, spread) ->
        val p = Path()
...
/** A four-point sparkle with a pinched waist, for the rare-finds readout. */
fun DrawScope.drawSparkle(color: Color, center: Offset, radius: Float) {
    val waist = radius * 0.22f
    val p = Path()
    p.moveTo(center.x, center.y - radius)
    p.cubicTo(center.x + waist, center.y - waist, center.x + waist, center.y - waist, center.x + radius, center.y)
    p.cubicTo(center.x + waist, center.y + waist, center.x + waist, center.y + waist, center.x, center.y + radius)
    p.cubicTo(center.x - waist, center.y + waist, center.x - waist, center.y + waist, center.x - radius, center.y)
    p.cubicTo(center.x - waist, center.y - waist, center.x - waist, center.y - waist, center.x, center.y - radius)
    p.close()
    drawPath(p, color)
...
/** A rosette: medal disc with ribbon tails, for the XP readout. */
...
/** A pointed-bottom heraldic shield. */
...
/**
 * A rank patch for a progression level. The marks escalate deliberately across the
 * ladder, so a patch says where you are without its label being read.
 *
 * [levelKey] matches `GeneratedProgressionConfig.levels`; unknown keys fall back to a paw.
 */
fun DrawScope.drawRankPatch(levelKey: String, border: Color, field: Color, art: Color) {
    val w = size.width
    val h = size.height
    ...
    when (levelKey) {
        ...
        "legendary_ranger" -> {
            ...
            repeat(9) { i ->
                ...
                drawLine(
                    art,
                    Offset(c.x + (u * 0.74f * cos(a)).toFloat(), c.y + (u * 0.74f * sin(a)).toFloat()),
                    Offset(c.x + (u * 0.96f * cos(a)).toFloat(), c.y + (u * 0.96f * sin(a)).toFloat()),
                    strokeWidth = u * 0.07f,
                    cap = StrokeCap.Round,
                )
            }
...
/** Drawn stat marks, used in readout rows in place of Material icons. */
enum class StatMark { DOT, TICK, SPARKLE, ROSETTE, PAW }
...
fun StatGlyph(mark: StatMark, color: Color, modifier: Modifier = Modifier) {
    Canvas(modifier) {
        ...
        val r = minOf(size.width, size.height) / 2f
        when (mark) {
            StatMark.DOT -> drawCircle(color, radius = r * 0.78f, center = c)
...
/** A drawn chevron, so a selector pill carries no Material icon. */
@Composable
fun ChevronGlyph(color: Color, modifier: Modifier = Modifier) {
    Canvas(modifier) {
        ...
        val left = Offset(w * 0.22f, h * 0.40f)
        val apex = Offset(w * 0.50f, h * 0.66f)
        val right = Offset(w * 0.78f, h * 0.40f)
        drawLine(color, left, apex, strokeWidth = stroke, cap = StrokeCap.Round)
        drawLine(color, apex, right, strokeWidth = stroke, cap = StrokeCap.Round)
    }
}

/**
 * A drawn cross, for clearing an input. Paired with [MagnifierGlyph] so a search field
 * carries no Material glyph at either end.
 */
@Composable
fun ClearGlyph(color: Color, modifier: Modifier = Modifier) {
    Canvas(modifier) {
        val r = minOf(size.width, size.height) / 2f
        val c = Offset(size.width / 2f, size.height / 2f)
        val arm = r * 0.62f
...
/** A drawn magnifier, so search fields carry no Material glyph. */
@Composable
